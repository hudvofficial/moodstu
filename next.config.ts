...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    // T-20260825: đã thử rút dynamic 180→30 với giả thuyết "giữ trang cũ nóng quá lâu
    // gây bão request" (xem agent/HANDOFFS/T-20260825-finance-nav-redirect-bug.spec.md) —
    // đo lại bằng 15 lần thử thật trên production: 7/15 vẫn tái hiện bug tự điều hướng,
    // TỆ HƠN baseline 2/8 trước đó. Timing tái hiện đều ở ~2s cho thấy staleTimes (vốn
    // chỉ ảnh hưởng thời gian cache được coi là "còn mới" để dùng lại, không phải thời
    // điểm request đang bay bị hủy) không phải đòn bẩy đúng — LÙI LẠI 180, không đánh
    // đổi hiệu năng cho 1 fix không có tác dụng đo được. Đang điều tra tiếp cơ chế
    // realtime (khớp mốc ~2s hơn).
    staleTimes: {
