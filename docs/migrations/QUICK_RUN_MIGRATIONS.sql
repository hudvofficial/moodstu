-- =====================================================
-- VENDOR EXPENSE TRACKING - ALL MIGRATIONS IN ONE FILE
-- Copy toàn bộ file này và paste vào Supabase SQL Editor
-- Chạy 1 lần là xong tất cả!
-- =====================================================

-- =====================================================
-- MIGRATION 1: Schema + Functions
-- =====================================================
...
BEGIN
  ...
  WHERE type = 'chi' AND category_code IN ('vendor', 'freelancer') AND deleted_at IS NULL
  LIMIT 1;
...
BEGIN
  ...
  SELECT full_name INTO v_vendor_name FROM public.vendors WHERE id = v_task.vendor_id AND deleted_at IS NULL;
  ...
  IF v_task.contract_id IS NOT NULL THEN
    SELECT contract_code INTO v_contract_code FROM public.contracts WHERE id = v_task.contract_id AND deleted_at IS NULL;
  END IF;
