...
| **M2b Dọn bảng cũ** ✅ 26/08 (user quyết chạy sớm hơn mốc 02/09) | DROP 4 view + 4 bảng `_legacy` + `record_vendor_payment_atomic` (`20260826130000_cashflow_m2b_drop_legacy.sql`); backup `docs/reports/backup_2026-08-26_*.json` | pre-check 31 = 26 + 5 ✓; snapshot trước = sau; `db:types` 0 ref; vault regen — spec `T-20260826-cashflow-m2b-drop-legacy` |
...
| **M3b Thiệp & kho UI** ✅ 26/08 | modal Xuất kho gợi ý "Bán thêm HĐ" khi SĐT khách lẻ trùng khách HĐ; ô chọn HĐ tìm theo SĐT; ô "Thiệp" trong Thao tác nhanh trang HĐ mở modal sẵn HĐ (Bán thêm HĐ / Xuất HĐ); Xuất HĐ revalidate trang HĐ + `/finance`. Không đụng DB (RPC có sẵn) | e2e `inventory-contract-sale`: bán thêm 10 tờ → hạng mục 11.000 + phiếu thu 11.000 + `stock_out` gắn HĐ, `cogs` 5.000; Xuất HĐ 5 tờ → `cogs` 7.500 hiện ở drawer lợi nhuận — spec `T-20260826-thiep-kho-ui` |
| **M5 Lương cứng** ✅ 27/08 | sổ kỳ cộng `employee_salaries.total_salary` (cột `monthly_salary` không ai ghi → trước luôn 0); trả lương qua `record_payee_payment_atomic('employee')` + phân bổ `employee_salary` (paid/remaining dẫn xuất, huỷ → nợ quay lại); `payable_items('employee')` gồm lương; sheet chỉ người có lương cơ bản; 3 RPC intelligence/forecast + chốt sổ/fallback không cộng sheet lên tiền | e2e `cashflow-m5`: sheet 3.000.000 → phải trả ekip +3.000.000, `cost_salary_base` +3.000.000; trả 1.000.000 → còn 2.000.000, huỷ → 3.000.000; UI tất toán → phiếu chi + phân bổ, két +3.000.000 — spec `T-20260827-luong-cung-m5`; user quyết `salary_info.base_salary` 100tr của Admin |
| **M4 Tiền vào** ✅ 27/08 | dashboard "Cần thu tiền" đọc `finance_pending_collections` (đến hạn = đã giao sản phẩm — một luật với `/finance`, hết "quá hạn" theo ngày chụp của `payment_plans`); generator lịch thu chỉ Cọc + Tất toán (bỏ Đợt 1/2 luôn 0đ), xoá 119 dòng installment không phân bổ (backup `docs/reports/backup_2026-08-27_payment_plans_installments.json`); nhãn Moodie. Khách HĐ mua thiệp qua "Bán thêm HĐ" đã làm ở M3b; "category thu" bỏ (chưa có báo cáo nào cần) | nhắc thu dashboard = đúng HĐ đã giao (1 HĐ, 3.300.000 — trước cảnh báo 20); phải thu đến hạn/chờ giao trước = sau; sức khoẻ thanh toán 8/8 = 0 — spec `T-20260827-tien-vao-payment-plans` |
