...
1. [x] **Lead Form Modal**: Create `components/crm/lead-form-modal.tsx` supporting CREATE and UPDATE modes. Ensure fields utilize SSOT components.
2. [x] **Detail Drawer**: Create `components/crm/lead-detail-drawer.tsx`. Design header (Name+Badge+Score), Summary details, and Action buttons (Edit, Convert, Mark Lost, Add Log).
3. [x] **Care Log**: Create `components/crm/lead-care-log.tsx` to display the atomic text-append history block.

## Verification
- [x] Modal behaves consistently using `openModal()` hook pattern.
- [x] Deletion correctly invokes `<ConfirmDialog>`.
- [x] CSS grep verification yields 0 violations.
