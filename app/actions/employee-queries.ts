...
export async function getEmployeeList(params: EmployeeListParams = {}) {
  return profileAction("employees.getEmployeeList", () =>
    withEmployeesAccess(async (supabase) => {
      ...
      const selectFields =
        "id, employee_code, full_name, department, position, role, phone, email, status, gender, avatar_url, start_date, deleted_at, updated_at";
  ...
  return profileAction("employees.getEmployeeStats", () =>
    withEmployeesAccess(async (supabase) => {
      ...
      if (rpcError && !isMissingRpcError(rpcError)) {
        throw new Error(`Lỗi tải thống kê: ${rpcError.message}`);
      }
      ...
      for (const employee of employees) {
        const dept = (employee.department as string) || "Khac";
