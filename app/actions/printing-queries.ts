...
export async function getContractOptions(
  ...
  return withAuth(async (supabase) => {
    ...
    if (customerIds.length > 0) {
      ...
      if (contractByCustomerError) {
        throw new Error(
          `Khong the tai hop dong theo khach hang: ${contractByCustomerError.message}`,
        );
      }

      (contractByCustomer ?? []).forEach((row) => contractIds.add(row.id));
    }
    ...
    if (error) {
      throw new Error(`Khong the tai goi y hop dong: ${error.message}`);
    }
