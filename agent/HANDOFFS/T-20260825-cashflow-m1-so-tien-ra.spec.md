...
**Merge + deploy (user: "merge + push luôn", 26/08):** ff `main` `5b0183d → d74d3cc`; Vercel `success`. **Production `stu.moodwedding.com`: Playwright `cashflow-m1.spec.ts` 4/4 PASS** (cùng 5 luồng RPC + smoke 5 màn, seed riêng dọn sạch — kiểm sau: 0 dòng E2E sót, `expenses` active vẫn 35, lab còn 1.905.000, integrity 0). Cửa sổ lệch DB↔app đã đóng.

**Còn lại (M2):** drop 2 bảng `_legacy` + 4 view sau khi prod chạy ổn 1 thời gian; dashboard 3 số; lãi/lỗ tháng theo luật ngày; màn công nợ hợp nhất.
