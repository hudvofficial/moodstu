...
| Production (sau merge) | **MERGED main `b46fac6 → 501274b`** (ff, user "ok merge + push"). Vercel: build Preview của `501274b` treo 41 phút chặn hàng đợi → user cancel → Production build lên. **Playwright `cashflow-m2` trên `stu.moodwedding.com`: 3/3 PASS** (delta T7/T8 4 RPC cùng số; history + void; UI 3 khối, `/finance/payables`, redirect, `/reports`). Sau test: expenses active 35, E2E 0, két T8 +203.600, lãi 37.090.000, phải trả 1.905.000, integrity 4/4 = 0. |
