...
- **Cửa sổ lệch DB↔app prod tới khi merge:** drawer "Sửa đơn" trên prod cũ vẫn gọi `getOrderPaymentHistory` → khối lịch sử báo lỗi tải (không chặn thao tác khác). Đã đóng khi deploy `61e338a`.
- **Sau merge (user "ok cho phép tiếp tục"):** ff `715de57 → 61e338a`, Vercel lên sau 4 phút. Prod lần 1: test 1 fail — dòng nhóm đơn SSR hiện trước khi React hydrate nên click đầu không mở drawer (local không gặp); sửa helper `openGroupDrawer` retry click bằng `expect(...).toPass()` → prod **3/3**, seed sạch (35 đơn in, E2E 0).
