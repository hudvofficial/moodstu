...
| `npm run verify:printing` | **đỏ** — nguyên nhân có sẵn (xem trên), 5/5 RPC read ok, integrity report chưa tới vì dừng ở check anon |
...
**Còn lại cần user:** (1) OK áp migration REVOKE `printing_stats` lên prod; (2) merge `claude/printing-drawer-fixes` → `main` + `git push origin main` (= deploy). Claude không tự push main / không tự đổi quyền DB prod.
