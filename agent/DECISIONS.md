...
- **Phụ lục M2 (2026-08-26, `T-20260826-cashflow-m2-ba-so`, uỷ quyền theo ADR này):**
  ...
  5. **App không đọc 4 view tương thích nữa** (đọc thẳng `expenses`/`expense_allocations` với `expenses!inner` lọc `deleted_at`; `printing_lab_overview` sửa cùng); migration **M2b** drop view + bảng `_legacy` + `record_vendor_payment_atomic` đã viết (`20260826130000_cashflow_m2b_drop_legacy.sql`), **áp ≥ 2026-09-02 khi user gật**.
  6. Ngoài scope, ghi nhận: `get_finance_intelligence`/`get_cashflow_forecast` (card sức khoẻ/runway/hoà vốn — số theo két, nhãn "biên lợi nhuận" chưa đúng) và snapshot chốt sổ `finance-close-actions.ts` vẫn công thức riêng → M2b/M5.
