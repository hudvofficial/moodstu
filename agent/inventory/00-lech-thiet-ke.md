...
| `calendar_month_events` phân quyền | lịch theo người | RPC **không có** `WHERE employee_id` — chỉ fallback TS lọc | `sale`/`media` thấy lịch **mọi** nhân sự |
...
| `contracts` 2 policy SELECT | `contracts_select` scope theo người tạo/được gán | policy thứ hai `contracts_authenticated_read USING is_active_employee()` **cộng OR** ⇒ scope vô hiệu | mọi nhân viên active đọc mọi HĐ qua client-direct |
