-- =====================================================
-- Printing Workflow Phase 1: Core Infrastructure
-- =====================================================
-- Features:
...
SELECT
  ...
  COALESCE(SUM(
    CASE
      ...
      WHEN op.payment_type = 'refund' THEN op.amount -- already negative in DB
...
-- ─── 8. UPDATE printing_orders status values ─────────

-- Add new statuses if needed (dat_coc, da_giao, hoan_thanh, huy_don)
-- Note: Existing status column might have constraints - check first
DO $$
BEGIN
  -- Try to update constraint if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name LIKE '%status%'
    AND table_name = 'printing_orders'
  ) THEN
    -- Add new statuses to existing constraint
    -- This is database-specific; adjust as needed
    RAISE NOTICE 'Please manually update printing_orders status constraint to include: dat_coc, da_giao, hoan_thanh, huy_don';
  END IF;
END $$;
