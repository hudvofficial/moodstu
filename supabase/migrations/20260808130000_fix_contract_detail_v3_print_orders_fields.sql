-- Fix: get_contract_detail_v3 LATERAL JOIN 7 (print_orders) thiếu 3 field mà
-- print-orders-block.tsx đọc: payment_status, items, print_file_url — v2 có đủ,
-- v3 viết lại làm rớt → mọi đơn in hiện "Chưa có SP" + "Rỗng" + "Thiếu file".
-- Bắt bởi E2E printing-ui-tablet.spec.ts. Cùng signature → CREATE OR REPLACE an toàn.
