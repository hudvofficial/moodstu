-- Calendar month event feed.
-- Keeps the current calendar formulas while reducing the main month load to one RPC.
...
BEGIN
  ...
  v_end := (
    CASE
      WHEN v_month >= 12 THEN make_date(v_year + 1, 1, 10)
      ELSE make_date(v_year, v_month + 1, 10)
    END
  );
  ...
  WHERE (
      wt.deadline >= v_start
      AND wt.deadline <= v_end
    )
    OR (
      wt.deadline IS NULL
      AND wt.start_date >= v_start
      AND wt.start_date <= v_end
    )
  ORDER BY event_date NULLS LAST, deadline NULLS LAST, start_date NULLS LAST;
END;
