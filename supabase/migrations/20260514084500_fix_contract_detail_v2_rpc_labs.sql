-- Fix get_contract_detail_v2 printing lab payload.
-- The labs table stores the display name in lab_name, while the app expects
-- print_orders[].labs.name. The previous RPC referenced l.name and failed.

CREATE OR REPLACE FUNCTION public.get_contract_detail_v2(p_contract_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER
