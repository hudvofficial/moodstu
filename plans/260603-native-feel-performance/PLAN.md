...
- ✅ **Phase 2 — Task 2.2 (Inventory)** — form **edit** optimistic patch + **create** đóng+revalidate (item_code auto-gen + stock_in_atomic) + drawer **delete** optimistic-remove (`removeFromListCache` — shape `{data,count}` như dress; server chặn nếu còn tồn/lịch sử → rollback an toàn). Stock in/out để nguyên (số server-computed). Verify: tsc 0 + eslint không thêm lỗi + page render OK. ⚠️ **Chưa click-test runtime** edit/delete (automation không mở được drawer do nested row buttons — KHÔNG phải lỗi code; cùng primitive+shape như dress đã verify) → **đề nghị spot-check thủ công**.
- ⏭️ **Tiếp theo:** 2.3 Services+Printing+Productivity · 2.4 Contracts (React Query — `setQueryData`) · 2.5 Leads+Calendar (dnd-kit) · 2.6 Finance (GIỮ revalidate).
