...
export default function HeartPasswordModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!password.trim()) {
      setError("Vui lòng nhập mật khẩu để thả tim.");
      return;
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-5 text-center">
          ...
          <h2 className="text-lg font-semibold text-text-primary">Mật khẩu thả tim</h2>
          <p className="mt-1 text-sm text-text-secondary">
            {galleryTitle || "Album này"} yêu cầu mật khẩu khi khách bấm thả tim.
          </p>
        </div>
