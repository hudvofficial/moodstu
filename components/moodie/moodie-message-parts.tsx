    ...
    <div className="space-y-3 pl-1">
      {parts.map((part, index) => {
        ...
        if (part.type === "timeline") {
          return (
            <section key={index} className="rounded-xl border border-border/70 bg-white p-3 shadow-xs">
                  ...
                  <div key={group.date} className="space-y-2">
                    ...
                    <div className="space-y-2 border-l border-border pl-3">
                      {group.items.map((item) => (
                        <div key={item.id} className="relative rounded-lg bg-bg-subtle px-3 py-2">
                          ...
                          <div className="mt-1.5 flex flex-wrap gap-1.5"><span className="rounded-full bg-white px-2 py-0.5 text-micro text-text-secondary">{item.source === "google" ? "Google" : item.source === "task" ? "Công việc" : "Studio"}</span>{item.status ? <span className="rounded-full bg-white px-2 py-0.5 text-micro text-text-secondary">{item.status}</span> : null}</div>
        ...
        if (part.type === "table") {
          return (
            <section key={index} className="overflow-hidden rounded-xl border border-border/70 bg-white shadow-xs">
              ...
              <div className="overflow-x-auto"><table className="w-full min-w-[520px] text-left text-caption"><thead className="bg-bg-subtle text-text-secondary"><tr>{part.columns.map((column) => <th key={column.key} className={`px-3 py-2 font-medium ${column.align === "right" ? "text-right" : column.align === "center" ? "text-center" : ""}`}>{column.label}</th>)}</tr></thead><tbody className="divide-y divide-border/50">{part.rows.map((row, rowIndex) => <tr key={rowIndex}>{part.columns.map((column) => <td key={column.key} className={`px-3 py-2 text-text-primary ${column.align === "right" ? "text-right" : column.align === "center" ? "text-center" : ""}`}>{formatCell(row[column.key] ?? null, column.format)}</td>)}</tr>)}</tbody></table></div>
        ...
        if (part.type === "gallery") {
          return (
            <section key={index} className="space-y-3 rounded-xl border border-border/70 bg-white p-3 shadow-xs">
        ...
        return (
          <section key={index} className="rounded-xl border border-border/70 bg-white p-3 shadow-xs">
            ...
            <div className="flex flex-wrap items-center gap-2">{part.nodes.map((node, nodeIndex) => <div key={node.id} className="flex items-center gap-2"><div className="min-w-32 rounded-lg border border-border bg-bg-subtle px-3 py-2 text-center"><p className="text-sm font-medium text-text-primary">{normalizeMoodieDisplayText(node.label)}</p>{node.subtitle ? <p className="text-caption text-text-muted">{node.subtitle}</p> : null}</div>{nodeIndex < part.nodes.length - 1 ? <ExternalLink className="h-3.5 w-3.5 rotate-[-45deg] text-text-muted" /> : null}</div>)}</div>
          </section>
        );
      })}
    </div>
