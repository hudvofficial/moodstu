...
export function BottomNav({ role, className }: BottomNavProps) {
  ...
  return (
    <>
      ...
      <nav
        className={cn(
          "lg:hidden fixed bottom-0 left-0 right-0 z-50 shrink-0 w-full bg-bg-card/90 backdrop-blur-lg border-t border-border shadow-bottom-nav px-1 py-1",
          className
        )}
      >
        <div className="flex items-center justify-around px-2 h-[56px] w-full">
          {navItems.map((item) => {
