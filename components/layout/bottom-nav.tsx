...
export function BottomNav({ role, className }: BottomNavProps) {
  ...
  return (
    <>
      ...
      <nav
        className={cn(
          "lg:hidden fixed bottom-0 left-0 right-0 z-50 shrink-0 w-full bg-bg-card/90 backdrop-blur-lg border-t border-border shadow-bottom-nav",
          className
        )}
        style={{ paddingBottom: 'max(0.5rem, env(safe-area-inset-bottom))' }}
      >
        <div className="flex items-center justify-around px-2 pt-1 pb-1 h-[60px] w-full">
          {navItems.map((item) => {
