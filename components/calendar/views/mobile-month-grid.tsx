                ...
                <div className="flex items-center justify-center gap-1 w-full mb-0.5 shrink-0">
                  <div className={`
                    ...
                    ${isToday ? "bg-primary text-white font-bold shadow-sm" : 
                      !isCurrentMonth ? "text-text-muted" : "text-text-main"}
                  `}
                    style={!isToday && isCurrentMonth && (date.getDay() === 0 || date.getDay() === 6) ? { color: "var(--color-text-weekend)" } : undefined}
                  >
                    {format(date, "d")}
                  </div>
                  <span
                    className={`text-micro leading-none ${isNewLunarMonth ? "font-bold text-text-main" : "text-text-muted"} ${!isCurrentMonth ? "opacity-60" : ""}`}
                    style={isNewLunarMonth && isCurrentMonth && !isToday ? { color: "var(--color-text-weekend)" } : undefined}
                  >
                    {lunarText}
                  </span>
                </div>
