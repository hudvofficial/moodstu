...
export function MobileMonthGrid({ currentDate, events, onDateSelect }: MobileMonthGridProps) {
  ...
  return (
    <div className="flex w-full flex-col overflow-hidden mb-8">
      {/* Header Days */}
      <div className="grid grid-cols-7 bg-bg-input">
        {["T2", "T3", "T4", "T5", "T6", "T7", "CN"].map((day) => (
          <div 
            key={day} 
            className={`py-2 text-center text-tiny font-bold text-text-muted uppercase tracking-wider
            `}
          >
            {day}
          </div>
        ...
        {daysInGrid.map((date) => {
          ...
          return (
            <div 
               ...
               className={`
                 relative flex flex-col items-center justify-start pt-2 cursor-pointer 
                 transition hover:bg-bg-hover active:bg-bg-hover
                 ${!isCurrentMonth ? "bg-bg-input/50 opacity-60" : "bg-bg-card"}
               `}
            >
              <div className={`
                flex items-center justify-center w-7 h-7 rounded-full text-sm font-medium
                ${isToday ? "bg-primary text-white font-bold shadow-sm" : "text-text-main"}
              `}>
                {format(date, "d")}
              </div>
                ...
                <div className="absolute flex flex-wrap gap-[3px] justify-center px-1 bottom-1.5 w-full">
                  {eventsForDay.slice(0, 3).map((e, i) => {
                    ...
                    const bgClass = e.colorToken.includes("rose") || e.colorToken.includes("pink") ? "bg-rose-400" : 
                                    ...
                                    e.colorToken.includes("slate") || e.colorToken.includes("gray") ? "bg-text-muted" : 
